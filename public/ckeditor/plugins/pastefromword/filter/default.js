...
(function () {
  ...
  var l = CKEDITOR.tools,
    ...
    G = 'v:arc v:curve v:line v:oval v:polyline v:rect v:roundrect v:group'.split(' '),
    B = {},
    z = 0,
    u = {},
    f,
  ...
  u.rules = function (c, b, d) {
    function e(a) {
      (a.attributes['o:gfxdata'] || 'v:group' === a.parent.name) && k.push(a.attributes.id);
    }
      ...
      x = {
        ...
        elements: {
          ...
          p: function (a) {
            ...
            if (f.thisIsAListItem(b, a))
              ...
              c &&
                ...
                (c.attributes['cke-list-level'] = e['mso-list'].match(/level(\d+)/)[1]);
          ...
          li: function (a) {
            ...
            g && ((a.attributes.style = h.normalizedStyles(a, b)), h.pushStylesLower(a));
          },
          ol: function (a) {
            if (g)
              return (
  ...
  u.lists = {
    ...
    setListStart: function (c) {
      ...
      switch (c.attributes['cke-list-style-type']) {
        ...
        case 'upper-alpha':
          c.attributes.start =
            f.getSubsectionSymbol(b[d]).replace(/\W/g, '').toLowerCase().charCodeAt(0) - 96 - d;
          break;
        case 'decimal':
          c.attributes.start = parseInt(f.getSubsectionSymbol(b[d]), 10) - d || 1;
      }
      ...
      toNumber: function (c, b) {
        ...
        function e(b) {
          ...
          for (var d = c.length, a = 0, e = 0; e < d; ++e)
            for (var f = c[e], q = f[1].length; b.substr(0, q) == f[1]; b = b.substr(q)) a += f[0];
          return a;
        }
        return 'decimal' == b
          ? Number(c)
    ...
    setListDir: function (c) {
      ...
      c.forEach(function (c) {
        'li' == c.name &&
          ('rtl' == (c.attributes.dir || c.attributes.DIR || '').toLowerCase() ? d++ : b++);
    ...
    createLists: function (c, b) {
      ...
      for (h = 0; h < r.length; h++) {
        ...
        for (a = 0; a < q.length; a++) {
          ...
          for (k = g.attributes['cke-list-level']; k > p.length; ) {
            ...
            0 < y.length
              ? y[y.length - 1].add(l)
              : ((y = new CKEDITOR.htmlParser.element('li', { style: 'list-style-type:none' })),
                y.add(l),
                m.add(y));
            p.push(l);
            C.push(l);
        ...
        p[0].children.length &&
          ...
          !m && 1 < p[0].children.length && (m = p[0].children[1].attributes['cke-symbol']),
      ...
      CKEDITOR.tools.array.forEach(n, function (b) {
        for (var a = [], c = b.parent; c; ) ('li' === c.name && a.push(c), (c = c.parent));
        var a = d(a),
          e;
        a &&
    ...
    calculateValue: function (c) {
      ...
      null === d &&
        ((d = void 0 !== b.attributes.start ? parseInt(b.attributes.start, 10) : 1), (e = 0));
      return d + (c - e);
    },
      ...
      for (var d = {}, e = [[]], g, k = 0; k < c.length; k++) {
        ...
        h
          ? ((r = h.type.match(/alpha/) && 7 == h.index ? 'alpha' : r),
            (r = 'o' == c[k].attributes['cke-symbol'] && 14 == h.index ? 'alpha' : r),
            (n = f.getSymbolInfo(c[k].attributes['cke-symbol'], r)),
            (a = this.getListItemInfo(c[k])),
            (h.type != n.type || (g && a.id != g.id && !this.isAListContinuation(c[k]))) &&
              e.push([]))
          : (n = f.getSymbolInfo(c[k].attributes['cke-symbol']));
        for (g = parseInt(c[k].attributes['cke-list-level'], 10) + 1; 20 > g; g++)
          d[g] && delete d[g];
    ...
    getSymbolInfo: function (c, b) {
      ...
      if (c.match(/\d/))
        return { index: c ? parseInt(f.getSubsectionSymbol(c), 10) : 0, type: 'decimal' };
      ...
      return (
        ...
        (b.attributes.style &&
          ...
          !!b.find(function (c) {
            ...
            var f = e['font-family'] || '';
            return (
              ((e.font || e['font-size'] || '').match(/7pt/i) && !!c.previous) || f.match(/symbol/i)
            );
          }, !0).length)
      );
    },
      ...
      if (this.isShifted(c)) {
        var b = CKEDITOR.tools.array.filter(c.children, function (b) {
          ...
          d = CKEDITOR.tools.array.reduce(
            ...
            function (b, c) {
              return (
                c.children && 1 == c.children.length && t.isShifted(c.children[0])
                  ? [c]
                  : c.children
              ).concat(b);
            },
