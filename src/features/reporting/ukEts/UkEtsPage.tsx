...
const UkEtsPage = () => {
  const [rowData, setRowData] = useState<UkData[]>([]);
  ...
  const gridRef = useRef<EtsGridRef<UkData>>(null);
  ...
  const handleSummaryExport = async () => {
    ...
    try {
      ...
      const fileName = `${currentDate}_uk_ets.xlsx`;
  ...
  const handleReportExport = async () => {
    ...
    callApiForFile({
      service: Service.POSTMAN,
      url: '/api/v1/reporting/ukets/report/excel',
      method: Method.GET,
      params: {
        queryParams,
      },
  ...
  const handleRawDataExport = async () => {
    ...
    callApiForFile({
      service: Service.POSTMAN,
      url: '/api/v1/reporting/ukets/raw/excel',
      method: Method.GET,
      params: {
        queryParams,
      },
