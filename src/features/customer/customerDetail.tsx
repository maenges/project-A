...
const CustomerDetail: React.FC = () => {
  ...
  const summary: Array<{ label: string; value: React.ReactNode }> = [
    { label: '회원 구분', value: detail?.user_type },
    { label: '회원 ID', value: detail?.user_id },
    { label: '닉네임', value: detail?.user_nick },
    { label: '보유머니', value: formatMoney(detail?.user_money) },
  ...
  const tabs: TabDef[] = useMemo(
    () => [
      ...
      { label: '보유머니이력', icon: <PaidOutlinedIcon fontSize="small" /> },
      { label: '충/환전이력', icon: <ReceiptLongOutlinedIcon fontSize="small" /> },
      { label: '게임이력', icon: <SportsEsportsOutlinedIcon fontSize="small" /> },
    ],
    []
  );
