...
function shouldSkipGeneratingVar(keys) {
  ...
  return (
    ...
    (keys[0] === 'palette' &&
      !!((_a = keys[1]) == null ? void 0 : _a.match(/(mode|contrastThreshold|tonalOffset)/)))
  );
}
...
function attachColorScheme(colorSchemes, scheme, restTheme, colorScheme) {
  ...
  const { palette, ...muiTheme } = createThemeNoVars_default({
    ...restTheme,
    palette: {
      mode,
      ...(scheme == null ? void 0 : scheme.palette),
...
function createThemeWithVars(options = {}, ...args) {
  ...
  if (!defaultScheme) {
    throw new Error(
      true
        ...
        : formatMuiErrorMessage(21, defaultColorScheme)
    );
  }
      ...
      if (color !== 'tonalOffset' && colors && typeof colors === 'object') {
        ...
        if (colors.contrastText) {
          setColor(
            palette[color],
            'contrastTextChannel',
            private_safeColorChannel(toRgb(colors.contrastText))
          );
        }
        if (color === 'text') {
  ...
  const { vars, generateThemeVars, generateStyleSheets } = prepareCssVars_default(
    theme,
    parserConfig
  );
  theme.vars = vars;
...
function createTheme(options = {}, ...args) {
  ...
  const defaultScheme =
    initialColorSchemes == null ? void 0 : initialColorSchemes[defaultColorSchemeInput];
...
//# sourceMappingURL=chunk-SCZ4OLDJ.js.map
